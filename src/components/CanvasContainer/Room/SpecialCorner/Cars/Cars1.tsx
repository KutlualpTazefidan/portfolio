...
export default function Cars1({ position, color, color2 }: any) {
  const { geometry: carGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Car.glb"
  );
  const { geometry: platesGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Plates.glb"
  );
  const { geometry: windowsGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Windows.glb"
  );
  const { geometry: bumperGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Bumper.glb"
  );
  const { geometry: rearLightsGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/RearLights.glb"
  );
  const { geometry: frontLights1Geom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/FrontLights1.glb"
  );
  const { geometry: frontLights2Geom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/FrontLights2.glb"
  );
  const { geometry: frontWheelsGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/FrontWheels.glb"
  );
  const { geometry: doorHandlesGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/DoorHandles.glb"
  );

  const { geometry: insideFrontGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/InsideFront.glb"
  );
  const { geometry: steeringGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Steering.glb"
  );
  const { geometry: rearMirrorCaseGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/RearMirrorCase.glb"
  );
  const { geometry: topmirrorsGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/RearMirror.glb"
  );
  const { geometry: sidemirrorsGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/SideMirrors.glb"
  );
  const { geometry: trunkGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Trunk.glb"
  );
  const { geometry: seatsGeom } = Import3dObject(
    "./assets/imports/Room/SpecialCorner/Cars/1/Seats.glb"
  );

  const color1Mat = new MeshPhysicalMaterial({
    color: color,
    metalness: 0.1,
    roughness: 0.8,
    reflectivity: 0.0,
  });
  const color2Mat = new MeshPhysicalMaterial({
    color: color2,
    metalness: 0.1,
    roughness: 0.8,
    reflectivity: 0.0,
  ...
  return (
    <>
      ...
      <mesh
        ...
        material={blackSaturnMat}
        scale={1}
        position={[position[0] - 0.034, position[1] + 0, position[2] + 0]}
        castShadow
        receiveShadow
      />
      ...
      <mesh
        ...
        material={blackSaturnMat}
        scale={1}
        position={[position[0] - 0.068, position[1] + 0, position[2] + 0]}
        castShadow
        receiveShadow
      />
      <mesh
        ...
        material={blackSaturnMat}
        scale={1}
        position={[position[0] - 0.1415, position[1] + 0, position[2] + 0]}
        castShadow
        receiveShadow
      />
      <mesh
